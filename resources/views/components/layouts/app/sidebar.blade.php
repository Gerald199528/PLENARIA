            ...
            document.addEventListener('livewire:init', () => {
                ...
                Livewire.on('swal:success', (event) => {
                    Swal.fire({
                        ...
                        text: event.text || 'Operación completada exitosamente.',
                        icon: 'success',
                ...
                Livewire.on('swal:error', (event) => {
                    if (event.toast) {
                        Swal.fire({
                            ...
                            text: event.text || 'Ha ocurrido un error.',
                            icon: event.icon || 'error',
                            toast: true,
                            position: event.position || 'top-end',
                            showConfirmButton: event.showConfirmButton || false,
                            timer: event.timer || 4000,
                            timerProgressBar: event.timerProgressBar || true
                        });
                    } else {
                        Swal.fire({
                            title: event.title || 'Error',
                ...
                Livewire.on('swal:warning', (event) => {
                    Swal.fire({
                        title: event.title || 'Advertencia',
                        text: event.text || 'Revisa la información.',
                        icon: 'warning',
                        timer: event.timer || 4000,
                        showConfirmButton: true,
                        confirmButtonText: 'Entendido'
                ...
                Livewire.on('swal:info', (event) => {
                    Swal.fire({
                        title: event.title || 'Información',
                        text: event.text || 'Información importante.',
                        icon: 'info',
                        timer: event.timer || 4000,
                        showConfirmButton: true,
                        confirmButtonText: 'Entendido'
