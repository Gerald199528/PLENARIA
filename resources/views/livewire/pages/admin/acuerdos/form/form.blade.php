<div class="bg-gray-100 dark:bg-gray-900 min-h-screen flex items-center justify-center py-6 sm:py-10 px-3 sm:px-4">
  ...
  <div class="w-full max-w-7xl bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden transition-all duration-500">
    ...
    <!-- Encabezado -->
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-700 to-indigo-900 text-white">
      <div class="flex items-center gap-2 sm:gap-3">
        ...
        <h2 class="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-extrabold tracking-wide">
          {{ $mode === 'edit' ? 'Editar Acuerdo' : 'Importar Acuerdo' }}
        </h2>
      </div>
...
<div class="p-3 sm:p-4 md:p-6 bg-blue-50 dark:bg-blue-900/30 border-l-4 border-blue-500 dark:border-blue-400">
  <div class="flex items-start gap-2 sm:gap-3">
    ...
    <p class="text-sm sm:text-base text-blue-700 dark:text-blue-200 font-medium leading-relaxed break-words">
      @if ($mode === 'edit')
        Aquí podrás <strong>editar solo la fecha de aprobación y observación</strong> de tu acuerdo.
      @else
    ...
    <form wire:submit.prevent="save" enctype="multipart/form-data" class="p-4 sm:p-6 md:p-8 space-y-6 md:space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        ...
        <div>
          ...
          <button type="button" 
                  onclick="window.open('{{ asset('storage/' . $acuerdo->ruta) }}', '_blank')"
      ...
      <div>
        ...
        <input type="text" 
               class="w-full p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-100 text-sm sm:text-base"
               value="{{ $acuerdo->categoria ? $acuerdo->categoria->nombre : 'Sin categoría' }}" readonly>
        <p class="text-yellow-500 text-xs sm:text-sm flex items-center gap-1 mt-1">
          <x-icon name="lock-closed" class="w-4 h-4" /> Campo bloqueado
        </p>
      </div>
      ...
      <div class="flex flex-col sm:flex-row justify-center gap-4 sm:gap-6 md:gap-8 pt-4 sm:pt-6 border-t border-gray-200 dark:border-gray-700">
        ...
        <x-button info type="submit" spinner="save" 
                  label="{{ $mode === 'edit' ? 'Guardar Cambios' : 'Guardar Acuerdo' }}" 
