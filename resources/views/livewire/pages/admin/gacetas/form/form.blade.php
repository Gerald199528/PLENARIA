    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-500 to-indigo-600 text-white">
      <div class="flex items-center gap-2 sm:gap-3">
        ...
        <h2 class="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-extrabold tracking-wide">
          {{ $mode === 'edit' ? 'Editar Gaceta' : 'Importar Gaceta' }}
        </h2>
      </div>
...
<div class="p-3 sm:p-4 md:p-6 bg-blue-50 dark:bg-blue-900/30 border-l-4 border-blue-500 dark:border-blue-400">
  <div class="flex items-start gap-2 sm:gap-3">
    <x-icon name="information-circle" class="w-5 h-5 sm:w-6 sm:h-6 text-blue-500 dark:text-blue-300 flex-shrink-0 mt-0.5 sm:mt-0" />
    <p class="text-sm sm:text-base text-blue-700 dark:text-blue-200 font-medium leading-relaxed break-words">
      @if ($mode === 'edit')
        Aquí podrás <strong>editar solo la fecha de aprobación y observación</strong> de tu gaceta.
      @else
    ...
    <form wire:submit.prevent="save" enctype="multipart/form-data" class="p-4 sm:p-6 md:p-8 space-y-6 md:space-y-8">
      ...
      @else
      <!-- MODO EDIT: Mostrar información (readonly) -->
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        <!-- Nombre -->
        <div>
        ...
        <div>
          ...
          <button type="button" 
                  onclick="window.open('{{ asset('storage/' . $gaceta->ruta) }}', '_blank')"
      ...
      <div>
        ...
        <input type="text" 
               class="w-full p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-100 text-sm sm:text-base"
               value="{{ $gaceta->fecha_importacion ? $gaceta->fecha_importacion->format('d/m/Y h:i A') : 'No registrada' }}" readonly>
        <p class="text-yellow-500 text-xs sm:text-sm flex items-center gap-1 mt-1">
          <x-icon name="lock-closed" class="w-4 h-4" /> Campo bloqueado
        </p>
      </div>
      ...
      <div>
        ...
        <input type="text" 
               class="w-full p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-100 text-sm sm:text-base"
               value="{{ $gaceta->categoria }}" readonly>
        <p class="text-yellow-500 text-xs sm:text-sm flex items-center gap-1 mt-1">
          <x-icon name="lock-closed" class="w-4 h-4" /> Campo bloqueado
        </p>
      </div>
      ...
      <div x-data="{ max: 1000, texto: @entangle('observacion') }">
        ...
        <textarea id="observacion" x-model="texto" maxlength="1000" rows="6" 
                  class="w-full p-3 sm:p-5 text-sm sm:text-base rounded-xl sm:rounded-2xl bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-100 resize-none 
                         placeholder-yellow-500 dark:placeholder-yellow-400 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all"
      ...
      <div class="flex flex-col sm:flex-row justify-center gap-4 sm:gap-6 md:gap-8 pt-4 sm:pt-6 border-t border-gray-200 dark:border-gray-700">
        ...
        <x-button info type="submit" spinner="save" 
                  label="{{ $mode === 'edit' ? 'Guardar Cambios' : 'Guardar Gaceta' }}" 
