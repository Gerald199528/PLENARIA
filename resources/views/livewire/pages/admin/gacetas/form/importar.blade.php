  <div class="bg-gray-100 dark:bg-gray-900 min-h-screen flex items-center justify-center py-6 sm:py-10 px-3 sm:px-4">
    ...
    <div class="w-full max-w-7xl bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden transition-all duration-500">
      ...
      <!-- Encabezado -->
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-500 to-indigo-600 text-white">
        <div class="flex items-center gap-2 sm:gap-3">
          <x-icon name="document-plus" class="w-6 h-6 sm:w-8 sm:h-8 text-white animate-bounce" />
          <h2 class="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-extrabold tracking-wide">
            Importar Gaceta
          </h2>
        </div>
        ...
        <x-button slate wire:click="cancel" spinner="cancel" label="Volver" icon="arrow-left"
          class="inline-flex items-center gap-2 px-3 py-2 text-xs sm:px-4 sm:py-2 sm:text-sm md:px-6 md:py-3 md:text-base
                bg-gradient-to-r from-gray-500 to-gray-600 text-white font-semibold rounded-lg sm:rounded-xl shadow-lg
                transform transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:from-gray-400 hover:to-gray-500
                focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400" />
      </div>

      <!-- Mensaje informativo -->
      <div class="p-4 sm:p-6 bg-blue-50 dark:bg-blue-900/30 border-l-4 border-blue-500 dark:border-blue-400">
        <p class="text-xs sm:text-sm md:text-base text-blue-700 dark:text-blue-200 font-medium flex items-start sm:items-center gap-2 leading-relaxed break-words">
          <x-icon name="information-circle" class="w-4 h-4 sm:w-5 sm:h-5 text-blue-500 dark:text-blue-300 flex-shrink-0" />
          Podrás <strong>importar tus gacetas</strong> de manera rápida y segura. Solo archivos PDF.
        </p>
      </div>

      <!-- Formulario -->
      <form wire:submit.prevent="save" enctype="multipart/form-data" class="p-4 sm:p-6 md:p-8 space-y-6 md:space-y-8">
       ...
       {{-- Fila 2: PDF y Botón Ver (si existe) --}}
            <div class="relative group">
                <label class="block text-base font-semibold text-gray-700 dark:text-gray-300 mb-2 flex items-center gap-2">
                    <x-icon name="paper-clip" class="w-5 h-5" />  Importar Gaceta (pdf)           
                     </label>        
        ...
        <div x-data="{ max: 1000, texto: @entangle('observacion') }" class="relative group">
          ...
          <textarea id="observacion" rows="6" x-model="texto" maxlength="1000"
            class="block w-full p-4 sm:p-5 text-sm sm:text-base text-gray-900 dark:text-gray-100 bg-gray-50 dark:bg-gray-700
                  border border-gray-300 dark:border-gray-600 rounded-xl sm:rounded-2xl resize-none
                  focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-300
                  group-hover:scale-[1.01] group-hover:shadow-lg"
            placeholder="Agrega cualquier detalle o nota relevante sobre la gaceta..."></textarea>

          <div class="flex flex-col sm:flex-row justify-between mt-2 text-xs sm:text-sm text-gray-500 dark:text-gray-400 gap-2 sm:gap-0">
        ...
        <!-- Botones -->
        <div class="flex flex-col sm:flex-row justify-center gap-4 sm:gap-8 pt-6 border-t border-gray-200 dark:border-gray-700">

          <!-- Botón Limpiar -->
          <x-button slate wire:click="limpiar" spinner="limpiar" label="Limpiar" icon="trash"
            class="inline-flex justify-center items-center gap-2 px-3 py-2 text-xs sm:px-5 sm:py-3 sm:text-sm md:px-6 md:text-base
                  bg-gradient-to-r from-gray-500 to-gray-600 text-white font-semibold rounded-lg sm:rounded-xl shadow-lg
                  transform transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:from-gray-400 hover:to-gray-500
                  focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 w-full sm:w-auto" />

          <!-- Botón Guardar -->
          <x-button info wire:click="save" spinner="save" label="Guardar Gaceta" icon="check"
            class="inline-flex justify-center items-center gap-2 px-3 py-2 text-xs sm:px-5 sm:py-3 sm:text-sm md:px-6 md:text-base
                  bg-gradient-to-r from-blue-600 to-indigo-500 text-white font-semibold rounded-lg sm:rounded-xl shadow-lg
