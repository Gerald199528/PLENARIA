        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-700 to-indigo-900 text-white gap-4">
            <div class="flex items-center gap-2 sm:gap-3">
                ...
                <h2 class="text-lg sm:text-2xl md:text-3xl lg:text-4xl font-extrabold tracking-wide">
                    {{ $mode === 'create' ? 'Registrar Sesión Ordinaria' : 'Editar Sesión Ordinaria' }}
                </h2>
            </div>
        ...
        <form wire:submit.prevent="save" enctype="multipart/form-data" class="p-4 sm:p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div class="relative group">
                    ...
                    <input 
                        type="text" 
                        wire:model="nombre" 
                        placeholder="Ejemplo: Sesión Ordinaria Nº 12"
            ...
            <div class="relative group">
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-end gap-2 sm:gap-3">
                    ...
                    {{-- Botón Ver PDF --}}
                    @if ($mode === 'edit' && isset($sesionOrdinaria) && $sesionOrdinaria->ruta)
                        <button type="button"
                            onclick="window.open('{{ asset('storage/' . $sesionOrdinaria->ruta) }}', '_blank')"
            ...
            <div class="flex flex-col sm:flex-row justify-center gap-2 sm:gap-6 pt-4 sm:pt-6 border-t border-gray-200 dark:border-gray-700">
                @if($mode == 'create')   
                    <x-button slate wire:click="limpiar" spinner="limpiar" label="Limpiar" icon="trash"
                        class="inline-flex items-center justify-center gap-2 px-4 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm bg-gradient-to-r from-gray-500 to-gray-600 text-white font-semibold rounded-xl shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:from-gray-400 hover:to-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400" />
                @endif                          
                <x-button info type="submit" spinner="save" label="{{ $mode === 'create' ? 'Guardar' : 'Guardar Cambios' }}" icon="check" 
                    class="inline-flex items-center justify-center gap-2 px-4 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm bg-gradient-to-r from-blue-600 to-indigo-500 text-white font-semibold rounded-xl shadow-lg hover:scale-105 transition-all duration-300" />
            </div>
        </form>
    </div>
</div>